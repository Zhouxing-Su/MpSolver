...
#ifndef CN_HUST_HAFV_UTIL_ARR_2D_H
#define CN_HUST_HAFV_UTIL_ARR_2D_H
...
public:
    ...
    explicit Arr2D(Index length1, Index length2) { allocate(length1, length2); }
    explicit Arr2D(Index length1, Index length2, T *data)
        : arr(data), len1(length1), len2(length2), len(length1 * length2) {}
    explicit Arr2D(Index length1, Index length2, const T &defaultValue) : Arr2D(length1, length2) {
...
#endif // CN_HUST_HAFV_UTIL_ARR_2D_H
